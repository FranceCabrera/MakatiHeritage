...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $reserve_date= $_POST['reserve_date_from_year'] . '-' . $_POST['reserve_date_from_month'] . '-' . $_POST['reserve_date_from_day'];
    $reserve_date_to = $_POST['reserve_date_to_year'] . '-' . $_POST['reserve_date_to_month'] . '-' . $_POST['reserve_date_to_day'];
    $room_capacity = $_POST['room_capacity'];
    $room_type = $_POST['room_type'];
    $payment_type = $_POST['payment_type'];
        if (!empty($name) && !empty($phone) && !empty($reserve_date) && !empty($reserve_date_to) && !empty($room_capacity) && !empty($room_type) && !empty($payment_type)) {
            $rate_per_day = 0;
            $additional_charge = 0;
            $discount = 0;
            ...
            $date_from = new DateTime($reserve_date);
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO reservationmillares (name, phone, reserve_date, reserve_date_to, room_capacity, room_type, payment_type)
                VALUES ('$name', '$phone', '$reserve_date', '$reserve_date_to', '$room_capacity', '$room_type', '$payment_type')";
...
<body>
    <header>
        <div class="header-container">
            <h1>MIGUI MILLARES FIVE STAR HOTEL ON-LINE REGISTRATION</h1>
        </div>
    </header>
    <div class="container">
        <nav>
            <ul>
                <li><a href="homeMillares.php">Home</a></li>
                <li><a href="profileMillares.php">Company's Profile</a></li>
                <li><a href="ReservationMillaresMigui.php">Reservations</a></li>
                <li><a href="contactsMillares.php">Contacts</a></li>
            </ul>
        </nav>
        <main>
            <div class="container-reservation-form">
                <h2>Reservation Page</h2>
                <form action="ReservationMillaresMigui.php" method="post">
